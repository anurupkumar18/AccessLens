...
describe('capture controller: pause, resume, stop, end (A5)', () => {
  ...
  it('stop() emits session.ended, releases the stream, and later frames never produce events', async () => {
    const { controller, stream, scheduler, events } = await sharing();
    stream.enqueue(demo('slide-01'));
    scheduler.tick(1);
    controller.stop();
    expect(types(events)).toEqual(['session.started', 'asset.changed', 'session.ended']);
  ...
  it('the browser ending the stream has the same effect as stop()', async () => {
    ...
    expect(types(events)).toEqual(['session.started', 'session.ended']);
    expect(controller.getState().phase).toBe('idle');
  ...
  it('start() after stop() reuses the open session and continues the sequence', async () => {
    ...
    expect(types(events)).toEqual(['session.started', 'session.ended', 'session.started']);
  ...
  it('endSession() while sharing stops first (session.ended) and then closes', async () => {
