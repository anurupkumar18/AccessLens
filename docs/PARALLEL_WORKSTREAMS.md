...
### Status — content and simulator block complete

Branch `workstream/5-content-camera-qa`. `make pack-check` validates it; the
guardrail suite is `tests/access_pack/`.

- Reviewed deck, fingerprints, reading order, regions, descriptions, and
  plain-language text: **done** — `packages/access-packs/bio-cell-demo/`, five
  slides, twelve regions.
- Original `.glb` cell model with stable node names: **done** — `models/cell.glb`,
  ten named organelle nodes, CC0, generated by `tools/generate_model.py`.
- Every demo `regionId` mapped to one AR hotspot and an equivalent semantic
  control: **done** — 12 regions, 12 hotspots, enforced by the validator.
- Event-sequence simulator for Parts 2, 3, and 4: **done** —
  `tools/simulate_events.py`, six ordered scenarios and ten single-fault
  rejection fixtures.
...
1. `arScene.hotspots[].cameraTarget`, a key into a new pack-level `arCameras` map
   ...
   region change, and that framing is reviewed content, so it belongs in the pack.
2. `arScene.hotspots[].highlight`, how the AR route marks the node. `"outline"`
   today.

Also: `hotspotId` is scoped per asset (`cell-slide-03:mitochondrion`) rather than
per region. Several slides teach the same region — cytoplasm appears on slides 01
and 03 — and a bare `mitochondrion-hotspot` would not resolve to one hotspot
pack-wide. The pack validator now fails on a duplicate `hotspotId`.
