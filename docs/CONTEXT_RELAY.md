...
| 3. Student experience and AR | Prachi | `feat/ar-shared-slide` | Student modes, direct Three.js/WebXR AR, semantic fallback, and local preferences are implemented. The AR surface is now pack-driven: every recognized asset with reviewed regions gets a synchronized spatial slide, while authored hotspots supply labels and unmatched content remains explicitly unmatched. | `node_modules/.bin/vitest.cmd run apps/extension/src/ar/PackArView.test.tsx apps/extension/src/ar/CellArView.test.tsx apps/extension/src/student/StudentExperience.test.tsx` (26 passed); `apps/extension/src/ar/PackArView.tsx` |
...
### RL-097 — 2026-09-17 — Part 3 — Codex

**Landed:** branched from current `master` at `2d69f4a` as `feat/ar-shared-slide` and replaced the student AR entry point's fixed cell scene with the pack-driven `apps/extension/src/ar/PackArView.tsx`. A recognized slide now renders its own reviewed regions as synchronized spatial hotspots, using authored `arScene` labels when present and a deterministic reviewed-region fallback otherwise. Immersive WebXR and keyboard/pointer controls remain available, while an unmatched slide shows an explicit no-invention state. `StudentExperience` now offers AR for any pack asset with reviewed regions; existing `CellArView` remains untouched for compatibility.

**Threads touched:** T-06 closed; no capture, transport, raw-media, or privacy contract changed.

**Next agent needs to know:** the repository-wide typecheck is currently blocked by pre-existing missing `@aws-sdk/client-bedrock-agent-runtime` and `@aws-sdk/client-sagemaker-runtime` modules in `services/ai-gateway`; the focused AR/student suite passes 26 tests. Real immersive WebXR still needs a compatible device smoke test.

### RL-098 — 2026-09-17 — Part 3 + authoring surface — Codex

**Landed:** added a localhost-only local slide preview path. When the dev build
has no authoring API or Google client configured, `AuthoringPanel` accepts a PNG
or JPEG, computes the same `dhash12` fingerprint used by capture, registers the
object URL locally, and hands the shell a device-local one-slide Access Pack.
The pack has one honest whole-slide placeholder region for spatial AR testing;
it is never sent to AWS or enabled in production builds. Authenticated PPTX/PDF
upload, review, and publish behavior is unchanged.

**Threads touched:** none; production Google ID-token authorization remains
required for the remote authoring API.

**Next agent needs to know:** local preview currently accepts slide images, not
PPTX/PDF parsing. Use an exported slide image to exercise local matching and AR;
the full deck pipeline still requires the authenticated authoring service.
