...
| T-33 | `liveRelayClient.subscribe` calls `LiveEventSchema.safeParse` and forwards only on success, with no else branch and no diagnostic, so any event the student client cannot parse vanishes silently. Combined with T-31 this produces exactly the failure AL-003 exists to prevent: a non-conforming `capture.stopped` is accepted by the relay, becomes the latest state, is discarded by every conforming student, and the student view stays "live" after sharing stopped. It also hides any future relay/extension schema drift. | Part 1 | Trust in the student status indicator | OPEN | `apps/extension/src/shell/liveRelayClient.ts:48`; AL-004's "False-live state" column is still "Not measured" |
...
### RL-035 — 2026-09-16 — cross-cutting — Claude
...
**Threads touched:** T-31, T-32, T-33 opened. T-21 stays IN PROGRESS with the
review box now ticked and three findings in its place.
**Next agent needs to know:** the review does **not** clear AL-003 for deployment
on its own. T-31 and T-33 compose into the one failure AL-003 exists to prevent —
a non-conforming `capture.stopped` is accepted relay-side, then silently dropped
by every conforming student, leaving the view "live" after sharing stopped — which
is the same cell AL-004's bench table still marks "Not measured". T-32 matters
because the bench's reconnect check exercises `join`, while the shipped client
reconnects through the untested `resume`. Deployment was not attempted: this
container has no AWS CLI and no authorized hackathon profile, and the ambient
`AWS_*` environment variables here are the sandbox's, not the team's account.
