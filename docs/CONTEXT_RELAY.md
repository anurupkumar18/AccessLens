...
| T-42 | **Hosted shell at `/app/` is behind the deployed relay.** `AccessLensLiveSession` (deployed 2026-09-16 from `integ/ui-api`) now returns `streamToken` on every capability; the hosted shell built before that parses capabilities with the strict `RoleCapabilitySchema`, so its create/join reject until `bash infra/scripts/deploy.sh` republishes the shell from a tree that carries `f50ce29` or later. Rolling the relay back is the other way out. Deploy held by the product owner's request on 2026-09-16. | Jacob | Live demo from the hosted shell | OPEN | Probe `services/live-session/scripts/probe-video.mjs` shows the field; the shell bundle at `/app/assets/index-DC96toqB.js` carries the strict schema |
...
### RL-043 — 2026-09-16 — Part 6 + Part 1 — Jacob
...
### RL-044 — 2026-09-16 — Parts 2, 3, 4 — Jacob
...
**Threads touched:** T-42 opened (hosted shell behind the relay; deploy
held at the product owner's request).
