...
| Live captions | Codex | **Ready for QA** | AL-048 widened the contract (closing T-16); AL-049 wires an instructor caption input and a bounded student transcript behind `captionsEnabled`. Unit-tested; instructor form needs a real `getDisplayMedia()` device. | Real-device QA per `docs/DEMO_PROOF_SPRINT.md`, including screen-reader behavior of the caption track. |
