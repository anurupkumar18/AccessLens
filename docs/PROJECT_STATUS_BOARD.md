...
| Canvas read-only course materials and RAG | Unassigned | **Blocked externally** | Design exists; no approved adapter, provider, retention policy, or institutional approval. | Approve allowlist/retention boundary, then build a disabled-by-default provider seam. |
| Bedrock gateway and reviewed authoring | Unassigned | **Blocked externally** | Existing pack-draft script is not a live reviewed-authoring flow. | Approve model/review policy and provider credentials, then add feature-flagged gateway. |
