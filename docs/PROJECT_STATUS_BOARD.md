...
| --- | --- | --- | --- | --- |
| Explicit tab/window/display capture | Codex (QA integration); Jacob (capture foundation); Omar (matching fix) | **Ready for QA** | `0347ad9` preserves browser activation, waits for instructor grant before session creation, and cleans up failed creation. AL-053 (integrated from `ui/blacksmith-revamp`) fixes window/screen slide matching -- previously only tab shares ever synced. | Run the Windows matrix; categorize any observed failure as AL-002. |
...
| AWS WebSocket relay | Omar Rizwan | **Blocked externally** | Service and client exist; public endpoint passes the old 19-event path but rejects `capture.stopped`. AL-055 (integrated from `ui/blacksmith-revamp`) fixes reconnect catch-up, extends retry to two minutes, and fixes a `close()` race that could drop the final `session.ended`. | Part 4 resolves lifecycle validator/redeploy, then run two-device bench. |
