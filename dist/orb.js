...
`)).slice(0,c),fromSelection:e.length>0}}var p=6e3;function m(){return window.location.hostname.endsWith(`instructure.com`)||document.querySelector(`.ic-app`)!==null}function h(e){let t=(window.getSelection()?.toString()??e??``).trim().replace(/\s+/g,` `),n=null;for(let e of[`#content`,`.user_content`,`.assignment-description`,`#syllabusContainer`]){let t=document.querySelector(e);if(t){n=t;break}}n=n??document.querySelector(`main`)??document.body;let r=[`#application`,`#header`,`#left-side`,`#right-side`,`.grading_box`,`.rubric_table`,`script`,`style`,`noscript`,`iframe`],i=``;if(t)i=t;else{let e=n.cloneNode(!0);r.forEach(t=>{e.querySelectorAll(t).forEach(e=>e.remove())}),e.querySelectorAll(`[aria-hidden="true"]`).forEach(e=>e.remove()),i=(e.textContent??``).trim().replace(/\s+/g,` `)}return{title:document.title,url:`${location.origin}${location.pathname}`,selection:t,text:i.slice(0,p),fromSelection:t.length>0}}function g(){let e=typeof window<`u`?window.speechSynthesis:void 0;return{get available(){return!!e},get speaking(){return!!e?.speaking},speak(t){if(!e||!t.trim())return;e.cancel();let n=new SpeechSynthesisUtterance(t);n.rate=.95,n.lang=document.documentElement.lang||`en`,e.speak(n)},stop(){e?.cancel()}}}var _=`data-accesslens-orb-mounted`,v={explain:`Explaining this page…`,simplify:`Putting this in simpler words…`,diagram:`Drawing a diagram…`},y=`https://ax7d57zgdz6yrxumcizsorlneq0feqkt.lambda-url.us-east-1.on.aws/`;async function b(){try{let e=typeof chrome<`u`?chrome?.storage?.local:void 0;if(e){let t=await e.get(`orbEndpoint`);if(typeof t.orbEndpoint==`string`&&t.orbEndpoint)return t.orbEndpoint}}catch{}return y}function x(){if(document.documentElement.hasAttribute(_)||document.querySelector(`meta[name="accesslens-app"]`))return;document.documentElement.setAttribute(_,``);let e=g(),n=``,r,o,c=async e=>{r?.abort(),r=new AbortController,o.setBusy(!0,v[e]);try{let i=m()?h(``):f();if(!i.text){o.showError(`There is no readable text on this page to explain.`);return}let s=await a(await b(),{mode:e,title:i.title,url:i.url,text:i.text},r.signal);n=t({...s,value:s.value.text}),o.showResult(s.notice,s.value.text,s.value.svg)}catch(e){if(e?.name===`AbortError`)return;o.showError(e instanceof i?e.message:`Could not reach the explanation service.`)}finally{o.setBusy(!1)}};o=s({onAction:e=>void c(e),onSpeak:()=>{let t=m()?h(``):f();e.speak(n||t.text.slice(0,4e3)),o.setSpeaking(!0)},onStopSpeaking:()=>{e.stop(),o.setSpeaking(!1)},onClose:()=>{e.stop(),o.setSpeaking(!1),o.close()}},e.available),typeof chrome<`u`&&chrome?.runtime?.onMessage?.addListener(e=>{e?.type===`accesslens:toggle-orb`&&(o.isOpen?o.close():o.open())})}document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,x,{once:!0}):x()})();