...
<body><main><h1>__TITLE__</h1><p class="hint">__SUMMARY__</p><svg id="visual" role="group" aria-label="__DESCRIPTION__"><title>__DESCRIPTION__</title>__SCENE__</svg><p id="readout" class="sr" aria-live="polite"></p><p class="hint">Keyboard: Right arrow or Space advances; Left arrow goes back; Tab reaches each node.</p><button id="previous" type="button">Previous</button><button id="next" type="button">Next</button></main>
<script>
(function(){
  ...
  function draw(){ const selected=highlighted||choices[step%Math.max(choices.length,1)]?.dataset.choice||'item-0';
    choices.forEach(choice=>{const active=choice.dataset.choice===selected;choice.classList.toggle('active',active);choice.setAttribute('aria-label',choice.dataset.choice+(active?' selected':''));choice.setAttribute('aria-pressed',String(active));});
  ...
  choices.forEach(choice=>{choice.addEventListener('click',()=>{highlighted=choice.dataset.choice;draw()});choice.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();highlighted=choice.dataset.choice;draw()}})});
...
for artifact_id, title, summary, subjects, tags, interaction in ITEMS:
    ...
    scene = scene_for(title, subjects, interaction).replace('class="choice"', 'class="choice" role="button"')
    html = TEMPLATE.replace('__TITLE__', title).replace('__SUMMARY__', summary).replace('__DESCRIPTION__', description).replace('__SCENE__', scene)
